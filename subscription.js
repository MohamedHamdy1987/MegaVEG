...
export async function checkSubscription() {
  ...
  const { data } = await supabase.from("subscriptions").select("*").eq("user_id", user.id).eq("active", true).single();
...
export async function createTrial(userId) {
  ...
  await supabase.from("subscriptions").insert({ user_id: userId, plan: "trial", start_date: today, end_date: end, active: true });
}
